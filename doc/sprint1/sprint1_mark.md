...
## Daily Stand-ups (max 15 marks)
  ...
  Daily Stand-ups Total Mark: 12 / 15

  Standup messages are post on a team-base, not by individual.
...
## User Stories (Tracked in JIRA) (max 10 marks) 
  ...
  - Broken down into tasks/subtasks (max 2 marks)
    ...
    Make sure to also assign all subtasks.
  ...
  - Prioritized	(max 2 marks)
    ...
    All the stories in Jira Backlog are medium priority, which don't match their priorities in associated milestones.
...
Some branch names and PR names are also misleading. For example branch `feature-signup` stands for both `SCRUM-3` signin and `SCRUM-1` signup; PR#4 "eature view posts page" also include story `SCRUM-17` delete listing; etc. And all the "listings" in Jira are "posts" in gitHub repo.
...
Idealy, each task has a separate branch. This enables task assignment among different people under the same user story.
